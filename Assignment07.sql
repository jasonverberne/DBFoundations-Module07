...
-- Change Log: 2024-08-14, Jason Verberne, Script Update
-- 2017-01-01,JasonVerberne,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JasonVerberne')
	 Begin 
	  Alter Database [Assignment07DB_JasonVerberne] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JasonVerberne;
	 End
	Create Database Assignment07DB_JasonVerberne;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JasonVerberne;
...
SELECT CategoryName
	, ProductName
	, FORMAT(UnitPrice, 'C', 'en-US') AS [UnitPrice]
FROM vProducts AS vp INNER JOIN vCategories AS vc
...
SELECT ProductName
	, DATENAME(mm, InventoryDate) + ', ' + LTRIM(RTRIM(str(DATEPART(yy, InventoryDate)))) AS [InventoryDate]
	, [Count] AS [InventoryCount]
FROM vInventories as vi INNER JOIN vProducts as vp
ON vp.ProductID = vi.ProductID
ORDER BY ProductName, CONVERT(DATETIME, InventoryDate);
...
AS 
	SELECT TOP 1000000 ProductName
		, DATENAME(mm, InventoryDate) + ', ' + LTRIM(RTRIM(str(DATEPART(yy, InventoryDate)))) AS [InventoryDate]
		, [Count] AS [InventoryCount]
	FROM dbo.vProducts AS vp INNER JOIN dbo.vInventories AS vi
	ON vp.ProductID = vi.ProductID
	ORDER BY ProductName, CONVERT(DATETIME, InventoryDate);
...
AS
	...
	ORDER BY CategoryName, CONVERT(DATETIME, InventoryDate);
...
AS
	SELECT TOP 1000000 ProductName
		,  InventoryDate
		, InventoryCount
		, IIF(InventoryDate LIKE ('January%'), 0, ISNULL(LAG(InventoryCount) OVER(ORDER BY ProductName, CONVERT(DATETIME, InventoryDate)), 0)) AS [PreviousMonthCount]
